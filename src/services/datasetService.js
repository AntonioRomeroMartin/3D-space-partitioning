 ...
 *
 * All datasets are cached after the first load so subsequent loads are instant.
 * URL-based datasets (bundled or arbitrary) are keyed by their URL string;
 * local File objects are keyed by `__local__:<filename>` and can be switched
 * back to from the UI without re-reading the file.
 *
 * @memberof services
...
export function createDatasetService({ scene, camera, controls }) {
  ...
  /**
   * Loads a point cloud from a URL string or a local File object.
   * Both are cached: URLs by their string value, File objects by `__local__:<filename>`.
   * @param {string|File} source
   */
  function load(source, onProgress) {
    const isFile = source instanceof File;
    const cacheKey = isFile ? `__local__:${source.name}` : source;
    ...
    return new Promise((resolve, reject) => {
      loadPointCloud(
        ...
        (points) => {
          ...
          // Direct typed-array scan avoids Box3/Vector3 method call overhead (O(n) but no allocation).
          const posArr = points.geometry.attributes.position.array;
          const nPts   = posArr.length / 3;
          let zMin = posArr[2], zMax = posArr[2];
